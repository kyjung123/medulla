...
struct MergeWriter {
  ...
  void book_nominal_once(TTree* out_tree, TTree* first_in_tree){
    ...
    std::unordered_set<std::string> skip = {
      ...
      "reco_leading_pion_cos_wrt_beam",
      "reco_leading_proton_cos_wrt_beam",
      "true_leading_pion_cos_wrt_beam",
      "true_leading_proton_cos_wrt_beam",
      "reco_leading_pion_ke",
      "reco_leading_proton_ke",
      "true_leading_pion_ke",
      "true_leading_proton_ke"
    };
  ...
  void bind_nominal_input(TTree* in_tree){
    ...
    reco_had_cos_src=-9999; true_had_cos_src=-9999; reco_had_ke_src=-9999; true_had_ke_src=-9999;
    ...
    const char* reco_cos_br = nullptr;
    const char* true_cos_br = nullptr;
    const char* reco_ke_br  = nullptr;
    const char* true_ke_br  = nullptr;

    if(use_pick_existing_hadron_branch){
      reco_cos_br = pick_existing(in_tree,
        "reco_leading_pion_cos_wrt_beam","reco_leading_proton_cos_wrt_beam");
      true_cos_br = pick_existing(in_tree,
        "true_leading_pion_cos_wrt_beam","true_leading_proton_cos_wrt_beam");
      reco_ke_br  = pick_existing(in_tree,
        "reco_leading_pion_ke","reco_leading_proton_ke");
      true_ke_br  = pick_existing(in_tree,
        "true_leading_pion_ke","true_leading_proton_ke");
    }
    else{
      // pick_existing OFF:
      // pion branch만 직접 사용하고, 없으면 그냥 -9999 유지
      if(in_tree->GetBranch("reco_leading_pion_cos_wrt_beam"))
        reco_cos_br = "reco_leading_pion_cos_wrt_beam";
      if(in_tree->GetBranch("true_leading_pion_cos_wrt_beam"))
        true_cos_br = "true_leading_pion_cos_wrt_beam";
      if(in_tree->GetBranch("reco_leading_pion_ke"))
        reco_ke_br = "reco_leading_pion_ke";
      if(in_tree->GetBranch("true_leading_pion_ke"))
        true_ke_br = "true_leading_pion_ke";
    }
